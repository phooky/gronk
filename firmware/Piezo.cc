...
// Tones are placed on a tone queue when setTone is called.  The sound queue is
// internally limited to TONE_QUEUE_SIZE.
//
// At some point later, runPiezoSlice, processes the tone
//
// Sounds are started and removed from the queue by runPiezoSlice (and also
// started by setTone if the queue is empty).
//
// There is no interrupt for this buzzer, the Timer is hooked up via OCR0B
// directly to the buzzer pin, and toggles when the timer loops.  This is
// frequency generation without the overhead of the CPU having to toggle the pin
// manually and run an interrupt at the frequency of the tone.  Reduces CPU load
// and results in better tone generation.
//
...
void processNextTone(void) {
  if (tones.isEmpty()) {
    ...
    if (tone.tone_data[FREQUENCY] == NOTE_0) {
      // Note 0 acts as a rest (i.e. doesn't play a note), so we shut the timer
      // down
      shutdown_timer();
    } else {
...
#ifdef MODEL_REPLICATOR2
      // Setup the counter to count from 0 to OCR0A and toggle OC0B on counter
      // reset
      if (freq >= 500) {
        // Prescaler = 8	(1 << 3)
      ...
      uint16_t outputCompareTop =
          (uint16_t)(((uint32_t)F_CPU / ((freq << (uint32_t)1)
                                         << (uint32_t)prescalerFactorBits)) -
                     1);
      TCCR4A = _BV(COM4A0); // Toggle OC0B on compare match (i.e we're running
                            // at half the frequency of ORC0A,

      TCCR4B = prescalerBits | // Prescaler
               _BV(WGM42);     // CTC (top == OCR0A)

      OCR4A =
          (uint16_t)outputCompareTop; // Frequency when compiled with prescaler
      ...
      uint8_t outputCompareTop =
          (uint8_t)(((uint32_t)F_CPU /
                     ((freq << (uint32_t)1) << (uint32_t)prescalerFactorBits)) -
                    1);
      // Setup the counter to count from 0 to OCR0A and toggle OC0B on counter
      // reset
      TCCR0A = _BV(COM0B0) | // Toggle OC0B on compare match (i.e we're running
                             // at half the frequency of ORC0A,
      ...
      OCR0A =
          (uint8_t)outputCompareTop; // Frequency when compiled with prescaler
...
const uint16_t
    tune_print_done[] PROGMEM =
        {
            // 1319,   85,
            // 0,      85,
            1568, 85, 0,    85,  2093, 85,  0,      85, 2637, 85,
            0,    85, 3136, 171, 0,    171, 2349,   85, 0,    85,
            2093, 85, 0,    85,  2637, 171, 0,      85, 0,    256,
            3520, 85, 0,    85,  2093, 342, NOTE_0, 0, // Zero duration is a
                                                       // terminator
};

const uint16_t tune_makerbot_tv[] PROGMEM = {
    NOTE_A7, 200,      NOTE_0,  20,     NOTE_A7, 200,     NOTE_0,
    20,      NOTE_AS7, 400,     NOTE_0, 20,      NOTE_C8, 400,
    NOTE_0,  20,       NOTE_A7, 800,    NOTE_0,  0, // Zero duration is a
                                                    // terminator
};

const uint16_t tune_beethoven_5th[] PROGMEM = {
    NOTE_FS6, 200, NOTE_0,   20,   NOTE_FS6, 200, NOTE_0,  20,  NOTE_FS6, 200,
    NOTE_0,   20,  NOTE_DS6, 1200, NOTE_0,   20,  NOTE_F6, 200, NOTE_0,   20,
    NOTE_F6,  200, NOTE_0,   20,   NOTE_F6,  200, NOTE_0,  20,  NOTE_D6,  1200,
    NOTE_0,   0, // Zero duration is a terminator
};
...
const uint16_t tune_start_print[] PROGMEM = {
    ...
    // 2489,   85,
    // 0,      85,
    831,  85, 0,   85, 3322, 85, 0,    85, 831,  85,  0,      85, 2489, 85,
    0,    85, 831, 85, 0,    85, 2349, 85, 0,    85,  831,    85, 0,    85,
    2794, 85, 0,   85, 831,  85, 0,    85, 2489, 171, NOTE_0, 0, // Zero
                                                                 // duration is
                                                                 // a terminator
};
...
const uint16_t tune_filament_start[] PROGMEM =
    {
        //  2217,   85,
        //  0,      85,
        1976, 85,   0,    85,   2489, 171,    0,    171, 1661, 85,   0,
        85,   1480, 85,   0,    85,   1976,   171,  0,   171,  1480, 85,
        0,    85,   1245, 85,   0,    85,     1480, 85,  0,    85,   1976,
        85,   0,    85,   1661, 342,  NOTE_0, 0, // Zero duration is a
                                                 // terminator
};

const uint16_t tune_error[] PROGMEM = {
    659, 105, 0,   105, 494,    105,
    0,   105, 294, 440, NOTE_0, 0, // Zero duration is a terminator
};

// Plays a tune given a tune id

void playTune(uint8_t tuneid) {
  const uint16_t *tunePtr = 0;
...
void startUpTone() { playTune(TUNE_FILAMENT_START); }
