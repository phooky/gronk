...
void processNextTone(void) {
    if (tones.isEmpty()) {
        ...
        if (tone.tone_data[FREQUENCY] == NOTE_0) {
            // Note 0 acts as a rest (i.e. doesn't play a note), so we shut the
            // timer down
            shutdown_timer();
        } else {
...
#ifdef MODEL_REPLICATOR2
            // Setup the counter to count from 0 to OCR0A and toggle OC0B on
            // counter reset
            if (freq >= 500) {
                // Prescaler = 8	(1 << 3)
            ...
            uint16_t outputCompareTop =
                (uint16_t)(((uint32_t)F_CPU /
                           ...
                           1);
            TCCR4A = _BV(COM4A0); // Toggle OC0B on compare match (i.e we're
                                  // running at half the frequency of ORC0A,

            TCCR4B = prescalerBits | // Prescaler
                     _BV(WGM42);     // CTC (top == OCR0A)

            OCR4A = (uint16_t)
                outputCompareTop; // Frequency when compiled with prescaler
            ...
            // Setup the counter to count from 0 to OCR0A and toggle OC0B on
            // counter reset
            TCCR0A = _BV(COM0B0) | // Toggle OC0B on compare match (i.e we're
                                   // running at half the frequency of ORC0A,
