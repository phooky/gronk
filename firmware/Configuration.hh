...
// This file details the pin assignments and features of the Makerbot Extended
// Motherboard 4.0
...
// The pin that connects to the active-low recieve enable line on the RS485
// chip.
...
// Variables are floats:  debug_onscreen1, debug_onscreen2 and can be found in
// Steppers.hh #define DEBUG_ONSCREEN

// If defined, the stack is painted with a value and the free sram reported in
// in the Version menu.  This enables debugging to see if the SRAM was ever
// exhausted which would lead to stack corruption. #define STACK_PAINT
...
// If false, each new line segment is started as if it was a new line segment,
// i.e. no prior history If true, each new line segment takes into account the
// phase of the last segment
...
// Minimum time in seconds that a movement needs to take if the planning
// pipeline command buffer is emptied. Increase this number if you see blobs
// while printing high speed & high detail. It will slowdown on the detailed
// stuff.
#define ACCELERATION_MIN_SEGMENT_TIME 0.0200

// Minimum planner junction speed (mm/sec). Sets the default minimum speed the
// planner plans for at the end of the buffer and all stops. This should not be
// much greater than zero and should only be changed if unwanted behavior is
// observed on a user's machine when running at very slow speeds. 2mm/sec is the
// recommended value.
#define ACCELERATION_MIN_PLANNER_SPEED 2

// Slowdown limit specifies what to do when the pipeline command buffer starts
// to empty. The pipeline command buffer is 16 commands in length, and Slowdown
// Limit can be set between 0 - 8 (half the buffer size).
//
// When Commands Left <= Slowdown Limit, the feed rate is progressively slowed
// down as the buffer becomes more empty.
//
// By slowing down the feed rate, you reduce the possibility of running out of
// commands, and creating a blob due to the stopped movement.
//
// Possible values are:
//
