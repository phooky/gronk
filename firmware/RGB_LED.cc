...
void init() {
  ...
  uint8_t data2[2] = {LED_REG_LEDOUT,
                      LED_INDIVIDUAL}; // & ( LED_RED | LED_GREEN | LED_BLUE)};
...
void setDefaultColor() {
  ...
  uint8_t LEDColor =
      1; // eeprom::getEeprom8(eeprom_offsets::LED_STRIP_SETTINGS, 1);
  uint32_t CustomColor =
      0xffffffff; // eeprom::getEeprom32(eeprom_offsets::LED_STRIP_SETTINGS +
                  // blink_eeprom_offsets::CUSTOM_COLOR, 0xFFFFFFFF);
...
void setLEDBlink(uint8_t rate) {
  ...
  if (rate > 0) {
    ...
    // turn group blink on
    uint8_t data[2] = {LED_REG_MODE2,
                       LED_OUT_INVERTED | LED_OUT_DRIVE | LED_GROUP_BLINK};
    TWI_write_data(LEDAddress, data, 2);

    _delay_us(50);

    uint8_t data2[2] = {LED_REG_LEDOUT,
                        LED_GROUP & (LED_RED | LED_GREEN | LED_BLUE)};
