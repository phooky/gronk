...
int16_t vel_from_js(int axis, bool inverted = false) {
  ...
  sei();
  if (pos < (JS_LOW_VAL + JS_MAX_ZONE))
    pos = JS_LOW_VAL;
  if (pos > (JS_HIGH_VAL + JS_MAX_ZONE))
    pos = JS_HIGH_VAL;
  pos -= JS_CENTER_VAL;
...
unsigned int read_analog(unsigned char channel) {
  ...
  ADCSRA |= 1 << ADSC;
  while (ADCSRA & (1 << ADSC)) {
  } // poll until analog read completes.
...
void init_timers() {
  ...
  // Reset and configure timer 2, the microsecond timer, advance_timer and debug
  // LED flasher timer.
